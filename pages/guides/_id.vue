<template>
   <div>
       <Navbar/>
    <div v-if="guide" class="w-1/2 bg-white shadow rounded-lg p-6 flex flex-col justify-center items-center m-auto mt-10">
  
      <h1 class="text-3xl font-semibold mb-4">{{ guide.title }}</h1>
      <img :src="guide.imageUrl" alt="Guide image" class="w-full h-64 object-cover mb-4 rounded md:h-full md:w-full" />
    ...
    <div v-if="$store.getters.isAdminOrRedacteur" class="mt-4">
        <button @click="editGuide" class="bg-blue-500 text-white px-4 py-2 rounded mr-2">Modifier</button>
        <button @click="deleteGuide" class="bg-red-500 text-white px-4 py-2 rounded">Supprimer</button>
      </div>
...
export default {
  ...
  async asyncData({ params, $axios }) {
    ...
    const headers = { Authorization: `Bearer ${authToken}` };
        ...
        await this.$axios.delete(`/api/v1/guides/${this.guide._id}`, {headers} );
